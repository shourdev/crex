...
fprintf(file,"} \n");
         ...
         fprintf(file,"   float %s%dfloatValue;\n",outputString,i2);
                fprintf(file,"    %s%dfloatValue = atof(%s); \n",outputString,i2,outputString);
                    fprintf(file,"  int %s%dintegerValue; \n",outputString,i2);
                      fprintf(file,"%s%dintegerValue = (int)%s%dfloatValue; \n",outputString,i2,outputString,i2);
        ...
        fprintf(file,"if (isStringOrInt(%s)==0) {\n",outputString);
                     fprintf(file,"%stype = \"int\";\n \n",outputString);        
        fprintf(file,"} \n");    

                 fprintf(file,"else{ \n");  
                  fprintf(file,"printf(\"Not a number. \\n\"); \n");
                   fprintf(file,"} \n");  
        fprintf(file,"} \n");
                i2++;
             }
             ...
             char* extractedText = strstr(line, "flt:") + strlen("flt:");
              strcpy(outputString, extractedText); 
                   fprintf(file," \n");
                      fprintf(file, "if (strcmp(%stype,\"int\")==0) { \n", outputString);
           fprintf(file,"   float %s%dfloatValue; \n",outputString,i3);           
           fprintf(file,"  %s%dfloatValue = atof(%s); \n",outputString,i3,outputString);
        
fprintf(file," int %s%dbufferSize = snprintf(NULL, 0, \"%%f\", %s%dfloatValue); \n",outputString,i3,outputString,i3);
fprintf(file,"char* %s%dtemp = (char*)realloc(%s, (%ssz + %s%dbufferSize + 1) * sizeof(char)); \n",outputString,i3,outputString,outputString,outputString,i3);
fprintf(file," %s = %s%dtemp; \n",outputString,outputString,i3);
fprintf(file," snprintf(%s, %s%dbufferSize + 1, \"%%f\", %s%dfloatValue); \n",outputString,outputString,i3,outputString,i3);
fprintf(file, "%stype = \"float\";\n", outputString);
fprintf(file,"} \n");
                  fprintf(file, "if (strcmp(%stype,\"str\")==0) { \n", outputString);
                  fprintf(file,"if (isDecimalOrString(%s)==0){ \n",outputString);
                        fprintf(file,"%stype = \"float\";\n \n",outputString);  
                                 
                  fprintf(file,"} \n");
                       fprintf(file,"else { \n");
                     fprintf(file,"printf(\"Not a float. \\n\"); \n");
                       fprintf(file,"} \n"); 
                         fprintf(file,"} \n");
                             fprintf(file, "if (strcmp(%stype,\"float\")==0) { \n", outputString);
                              fprintf(file,"%stype = \"float\";\n \n",outputString);  
            ...
            if (strstr(line, "prtype:") != NULL) {
                     char* extractedText = strstr(line, "prtype:") + strlen("prtype:");
        strcpy(outputString, extractedText); 
                        remove_leading_whitespace(outputString);
        
            fprintf(file,"printf(\"%%s\",%stype); \n",outputString);
             }
        line = strtok(NULL, "\n");
...
strcpy(acname,name);
  ...
  fprintf(file, "// Generated by the crex compiler. \n");
    fprintf(file, "#include <stdio.h>\n");
      fprintf(file, "#include <stdlib.h>\n");
           fprintf(file, "#include <string.h>\n");
                      fprintf(file, "#include <ctype.h>\n");
fprintf(file,"#include <stdbool.h> \n");
fprintf(file,"int isStringOrInt(const char* input) { \n");
fprintf(file,"  int i, len; \n");
fprintf(file,"len = strlen(input);\n");
fprintf(file,"   bool isInteger = true;\n");
fprintf(file," for (i = 0; i < len; i++) {\n");
fprintf(file,"   if (input[i] < '0' || input[i] > '9') {\n");
fprintf(file," isInteger = false;\n");
fprintf(file,"  break;\n");
fprintf(file,"  }\n");
fprintf(file,"  }\n");
fprintf(file,"    if (isInteger){\n");
...
fprintf(file,"  }\n");
fprintf(file,"  }\n");
fprintf(file,"  int isDecimalOrString(const char* str) {\n");

fprintf(file,"  char* endptr;\n");
fprintf(file,"     strtod(str, &endptr);\n");
